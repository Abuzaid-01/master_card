...
def evaluate_round_comparison(
    ...
    if "text" in round2_models and "text" in partitions:
        ...
        df_eval = partitions["text"]["df_eval"].copy()
        df_eval_fraud = df_eval[df_eval["is_fraud"] == 1].copy()
        df_eval_legit = df_eval[df_eval["is_fraud"] == 0].copy()

        r1_detector = prober._get_text_detector()
        r2_detector = round2_models["text"]["detector"]

        # Calibrated decision thresholds
        r1_th = float(getattr(r1_detector, "optimal_threshold", 0.70))
        r2_th = float(getattr(r2_detector, "optimal_threshold", 0.70))
        text_baseline = {}

        if "text" in baseline_val_data:
            df_base = baseline_val_data["text"].copy()
            y_base = df_base["is_fraud"].values
            
            r1_base_probs = r1_detector.predict_proba_semantic(df_base) if r1_detector else np.zeros(len(df_base))
            r2_base_probs = r2_detector.predict_proba_semantic(df_base) if r2_detector else np.zeros(len(df_base))

            r1_base_fpr = float(np.round((r1_base_probs[y_base == 0] >= r1_th).mean(), 4))
            r2_base_fpr = float(np.round((r2_base_probs[y_base == 0] >= r2_th).mean(), 4))
            r1_base_auc = float(np.round(average_precision_score(y_base, r1_base_probs), 4)) if len(np.unique(y_base)) > 1 else 1.0
            r2_base_auc = float(np.round(average_precision_score(y_base, r2_base_probs), 4)) if len(np.unique(y_base)) > 1 else 1.0
        ...
        # Independently perturb eval fraud with alternate strategy seed against R1 threshold
        df_eval_fraud_adv = prober.probe_text(
            df_eval_fraud, threshold=r1_th, strategy_seed=eval_strategy_seed
        )
        ...
        if r1_detector and hasattr(r1_detector, "predict_proba_semantic"):
            r1_probs = r1_detector.predict_proba_semantic(df_eval_adv)
        else:
            r1_probs = np.zeros(len(df_eval_adv))

        if r2_detector and hasattr(r2_detector, "predict_proba_semantic"):
            r2_probs = r2_detector.predict_proba_semantic(df_eval_adv)
        else:
            r2_probs = np.zeros(len(df_eval_adv))
        ...
        report["text"] = {
            ...
            "catastrophic_forgetting_check": text_baseline,
        }
        print(f"      -> Adversarial: R1 caught {r1_caught}/{r1_total_fraud} (th={r1_th}) | R2 caught {r2_caught}/{r1_total_fraud} (th={r2_th}) | Δ={r2_caught-r1_caught:+d}")
        print(f"      -> AUC-PR: R1={r1_auc} → R2={r2_auc} (Δ={r2_auc-r1_auc:+.4f})")
